...
注意：小地图不显示敌对单位，敌人检测靠 enemy_detector.py（降落后看血条）
...
def best_node(nodes: list[NodeInfo]) -> Optional[NodeInfo]:
    """返回距中心最近的矿点。"""
...
def debug_draw(
    ...
    for node in nodes:
        cv2.circle(out, (node.pixel_x, node.pixel_y), 5, (0, 255, 255), 2)
        cv2.line(out, (cx, cy), (node.pixel_x, node.pixel_y), (0, 255, 255), 1)
